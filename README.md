...
Each service adds the hostname from its own configured `url` to its TLS certificate, alongside `localhost`, `127.0.0.1`, `::1`, the Android emulator host alias `10.0.2.2`, and the host's LAN addresses. The certificates chain to the locally generated root CA (`<data_dir>/tls/ca-cert.pem`), so a client either trusts that root CA or uses the Wallet Solution's explicitly test-only unsafe-TLS mode. They are not suitable for production use.
...
└── tls/
    ├── ca-key.pem                 # TLS root CA private key
...
Until `tls/ca-cert.pem` is trusted by the device under test, use a Wallet Solution's explicitly test-only unsafe-TLS mode where supported.
...
| `[wallet]`            | `auth_base_uri`                   | Absolute URI, any scheme                             | Base URI presentation engagements are built on. Default: `openid4vp://`. Replace it with the wallet's own scheme or a claimed https universal link.    |
...
Each presentation scenario declares how it hands its engagement to the wallet, and the runner follows that declaration:

| Declared delivery | What the runner does                                                                                                                                                     |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `qr`              | Prints the engagement as a scannable QR code in the terminal and opens `https://<relying-party>/presentation-request` in the default browser for a larger, crisper code. |
| `deep-link`       | Prints the engagement URI, for the tester to transfer to the wallet's device.                                                                                            |

The engagement URI is copied to the clipboard either way, so a device the tool cannot reach is never a dead end.

Every presentation scenario engages the wallet through `openid4vp://?client_id=…&request_uri=…`, the OpenID4VP custom scheme. A Wallet Solution that registers a different scheme, or that is launched through an https universal link, is engaged by setting `[wallet] auth_base_uri`; the same `client_id` and `request_uri` parameters are appended to whatever base URI is configured.

A QR engagement is scanned by a _second_ device, which cannot reach `127.0.0.1`. The runner warns when a scenario's `request_uri` points at a loopback host; to run those flows, set the service URLs in `config.ini` to this machine's LAN address and make the scanning device trust the local root CA.
