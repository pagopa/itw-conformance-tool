...
export function renderCheckCard(check: ConformanceCheck): string {
  ...
  if (result === 'FAIL') {
    const problemText = escapeHtml(errorMessage ?? 'Verifica fallita senza dettagli di errore.');
    return `
...
export function renderComplianceBanner({ compliancePct, complianceTier, statusLabel }: ComplianceBannerData): string {
  return `
...
export function renderFooter({ generatedAt, reportId, sessionId }: FooterData): string {
  const sessionLine = sessionId !== undefined ? `<p>ID Sessione: ${escapeHtml(sessionId)}</p>` : '';
  return `
    <footer class="footer">
 ...
 */
export function renderPrintTabBar(activeView: 'executive' | 'technical'): string {
