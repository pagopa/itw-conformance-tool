...
import { renderComplianceBanner, renderDetailsGrid, renderFooter, renderHeader, renderPrintTabBar } from './shared.js';

import type { ReportData } from './types.js';

// ─── Executive view ───────────────────────────────────────────────────────────

export function renderExecutiveView(data: ReportData, showPrintTabBar = false): string {
  const {
    compliancePct,
    complianceTier,
    criticalChecks,
  ...
  const partialVerb = partialCount === 1 ? 'è parzialmente conforme' : 'sono parzialmente conformi';
