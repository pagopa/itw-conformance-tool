import { readPackageVersion } from '@itw-conformance-tool/utils';

import { renderExecutiveView } from './executive-view.js';
import {
  formatDateTime,
  formatGeneratedAt,
...
import { renderCheckCard } from './shared.js';
import { REPORT_CSS } from './styles.js';
import { renderTechnicalView } from './technical-view.js';

import type { ConformanceSession } from '../types.js';
import type { ReportData, ReportView } from './types.js';
import type { ConfigSchemaType } from '@itw-conformance-tool/config';

// ─── Public API ───────────────────────────────────────────────────────────────

export function renderHtml(session: ConformanceSession, config?: ConfigSchemaType, view: ReportView = 'both'): string {
...
function buildReportData(session: ConformanceSession, config?: ConfigSchemaType): ReportData {
...
function renderSingleView(view: 'executive' | 'technical', data: ReportData): string {
  const content = view === 'executive' ? renderExecutiveView(data, false) : renderTechnicalView(data, false);
