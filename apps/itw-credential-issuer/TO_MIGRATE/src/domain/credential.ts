import { CallbackContext, JwtPayload, Oauth2Error, verifyTokenDPoP } from '@pagopa/io-wallet-oauth2';
import {
...
import { HttpMethod, IoWalletSdkConfig, ItWalletSpecsVersion } from '@pagopa/io-wallet-utils';
 ...
 */
export const createCredential = async (options: CreateCredentialOptions): Promise<CreateCredentialResponseResult> => {
  ...
  const credentialResponse = await buildCredentialResponse(options.config, credential);

  return credentialResponse;
};

const verifyCredentialProof = async (options: CreateCredentialOptions, expectedNonce: string, jwt: string) => {
...
const createCredentialByConfiguration = async (
  ...
  if (credentialIdentifier === 'dc_sd_jwt_PersonIdentificationData') {
    return await createPidCredential(baseURL, jwksRepository, holderPublicKey, config, fakeUser);
  }

  if (credentialIdentifier === 'dc_sd_jwt_EuropeanDisabilityCard') {
    return await createDisabilityCardCredential(baseURL, jwksRepository, holderPublicKey, config, fakeUser);
  }
  ...
  if (
    ...
    return await createMdocCredential(document, jwksRepository, holderPublicKey);
  }

  throw new CreateCredentialError(`Credential Identifier ${credentialIdentifier} not found`);
};
