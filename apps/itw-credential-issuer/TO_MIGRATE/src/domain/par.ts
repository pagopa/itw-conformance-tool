...
export interface ParRequestRepository {
  ...
  readonly get: (query: { code?: string; jti?: string; requestUri?: string }) => Promise<ParRequest>;
...
export const verifyAndSaveParRequest = async (options: VerifyAndSaveParRequestOptions): Promise<string> => {
  ...
  const { authorizationRequest, authorizationRequestJwt, clientAttestation } = await parsePushedAuthorizationRequest({
