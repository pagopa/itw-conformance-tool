...
const exportCoseKey = async (key: CryptoKey): Promise<CoseKey> =>
  CoseKey.fromJwk((await crypto.subtle.exportKey('jwk', key)) as Record<string, unknown>);
...
const createCertificateChain = async (now: Date) => {
  ...
  return {
    holderPublicJwk: (await crypto.subtle.exportKey('jwk', (await generateEcKeyPair()).publicKey)) as Record<
      string,
      unknown
    >,
    ...
    x5chain: [new Uint8Array(leafCertificate.rawData), new Uint8Array(rootCertificate.rawData)] as [
      Uint8Array,
      Uint8Array
    ]
  };
...
const issueCredential = async (
  ...
  const { holderPublicJwk, issuerKey, trustedCertificate, x5chain } = await createCertificateChain(now);
