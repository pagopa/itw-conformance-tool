...
describe('createOid4VpSessionTranscript', () => {
  it('builds the OpenID4VP handover transcript for encrypted responses', async () => {
    ...
    const thumbprintBytes = new Uint8Array(Buffer.from(thumbprint, 'base64url'));
    ...
    expect(cborDecode(transcript)).toEqual([null, null, ['OpenID4VPHandover', expectedHash]]);
  });
  ...
  it('uses null as the JWK thumbprint when no verifier key is provided', async () => {
    ...
    const expectedHandoverInfo = ['client-id', 'nonce-123', null, 'https://issuer.example/redirect'];
    const expectedHash = new Uint8Array(
      createHash('sha256')
        .update(cborEncode(DataItem.fromData(expectedHandoverInfo)))
        .digest()
