...
describe("verifyAndSaveParRequest", () => {
  ...
  it("should successfully verify and save a PAR request", async () => {
    ...
    vi.mocked(randomUUID)
      .mockReturnValueOnce(
        mockGeneratedRequestUri as ReturnType<typeof randomUUID>,
      )
    ...
    vi.mocked(verifyPushedAuthorizationRequest).mockResolvedValue(
      undefined as never,
    );
    ...
    expect(mockParRequestRepository.insert).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        id: mockGeneratedId,
        request_uri:
          "urn:ietf:params:oauth:request_uri:" + mockGeneratedRequestUri,
      }),
    );
    expect(result).toBe(
      "urn:ietf:params:oauth:request_uri:" + mockGeneratedRequestUri,
  ...
  it("should throw an error if oauth.verifyPushedAuthorizationRequest fails", async () => {
    ...
    vi.mocked(randomUUID).mockReturnValueOnce(
      "a1b2c3d4-e5f6-7890-1234-567890abcdef" as ReturnType<typeof randomUUID>,
    );
    ...
    vi.mocked(verifyPushedAuthorizationRequest).mockRejectedValue(
