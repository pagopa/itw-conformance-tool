...
vi.mock("../openid-federation", async (importOriginal) => {
  ...
  return {
    ...actual,
    getFederationMetadata: vi
      .fn()
      .mockResolvedValue("mock-federation-metadata"),
  };
});
...
vi.mock("@pagopa/io-wallet-oid4vp", async (importOriginal) => {
  ...
  return {
    ...
    createAuthorizationRequest: vi.fn(
      async ({ authorizationRequestPayload, callbacks, jar }) => {
        const { jwt } = await callbacks.signJwt(jar.jwtSigner, {
          header: {
            kid: jar.jwtSigner.kid,
            typ: "oauth-authz-req+jwt",
            ...(jar.jwtSigner.method === "x5c"
              ? { x5c: jar.jwtSigner.x5c }
              : { trust_chain: jar.jwtSigner.trustChain }),
          },
...
describe("handleEAAAuthorizationResponse", () => {
  it("returns a v1.3 request object with kid and x5c in the header", async () => {
    ...
    const response = await handleEAAAuthorizationResponse(
      context,
      parRequest,
      requestUri,
    );
