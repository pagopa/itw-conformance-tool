...
vi.mock('@pagopa/io-wallet-oid4vci', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@pagopa/io-wallet-oid4vci')>();
  return {
    ...actual,
...
vi.mock('../z-jwk', () => ({
  JwkPublicKey: {
    safeParse: vi.fn().mockReturnValue({ data: { kid: 'holder-kid' }, success: true })
  }
}));
...
describe('createCredential()', () => {
  ...
  it('passes trusted wallet provider issuers for V1_3 proof verification', async () => {
    ...
    expect(verifyCredentialRequestJwtProof).toHaveBeenCalledWith(
      expect.objectContaining({
        config: configV13,
        trustedWalletProviderIssuers: ['https://wallet-provider.example', 'https://wallet-provider.wct.example:3002']
      })
    );
  });
  ...
  it('rejects a DPoP proof with htm=GET', async () => {
    ...
    vi.mocked(decodeJwt)
      ...
      .mockReturnValueOnce({ nonce: 'nonce-123' });
    vi.mocked(verifyTokenDPoP).mockRejectedValue(new Error('htm mismatch: expected POST, got GET'));
  ...
  it('rejects a DPoP proof with a missing or wrong ath claim', async () => {
    ...
    vi.mocked(verifyTokenDPoP).mockRejectedValue(
      new Error("Dpop jwt does not have a ath value, but expected ath value 'abc123'.")
    );
  ...
  it('rejects a DPoP proof that exposes a private key', async () => {
    ...
    ).rejects.toThrowError('Private keys are not allowed in the DPoP Proof JWT!');
  });
});
