...
describe('makeParRequestRepository.consumeByCode', () => {
  ...
  it('should reject codes without expiry metadata', async () => {
    ...
    await expect(result).rejects.toThrowError(/Authorization code is missing expiry metadata: code123/);
  });
  ...
  it('should reject expired codes', async () => {
    ...
    await expect(result).rejects.toThrowError(InvalidGrantError);
    await expect(result).rejects.toThrowError(/Authorization code has expired at: 1700000000/);
  });
