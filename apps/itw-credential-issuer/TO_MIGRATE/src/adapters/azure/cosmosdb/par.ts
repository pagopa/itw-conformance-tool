...
export const makeParRequestRepository = (db: Database, containerName = 'par-requests'): ParRequestRepository => {
  ...
  return {
    async consumeByCode(code: string): Promise<ParRequest> {
      ...
      if (resources.length === 0) {
        throw new InvalidGrantError(`No Pushed Authorization Request found for code: ${code}`);
      }
      ...
      if (storedParRequest.code_consumed_at !== undefined) {
        throw new InvalidGrantError(`Authorization code has already been used: ${code}`);
      }

      if (storedParRequest.code_expires_at === undefined) {
        throw new InvalidGrantError(`Authorization code is missing expiry metadata: ${code}`);
      }

      const currentTime = Math.floor(Date.now() / 1000);
      if (currentTime > storedParRequest.code_expires_at) {
        throw new InvalidGrantError(`Authorization code has expired at: ${storedParRequest.code_expires_at}`);
      }
    ...
    async get(query: { code?: string; jti?: string; requestUri?: string }): Promise<ParRequest> {
      const querySpec = {
        parameters: [
          {
            name: '@requestUri',
      ...
      if (resources.length === 0) {
        throw new Error(`No item found with request_uri: ${query.requestUri} or code: ${query.code}`);
      }
