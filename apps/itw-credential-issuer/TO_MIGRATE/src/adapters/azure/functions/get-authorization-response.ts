...
import { handleEAAAuthorizationResponse, handlePIDAuthorizationResponse } from '@/domain/authorization';
import { SupportedCredentialsId } from '@/domain/z-credential';

import { createErrorLocationResponse, createErrorResponse, createGenericErrorResponse } from './errors/error';

export const GetAuthorizationResponseHandler: HttpHandler = async (request, context) => {
  ...
  try {
    ...
    if (clientId !== parRequest.client_id) {
      return createErrorLocationResponse(parRequest.redirect_uri, parRequest.state, {
        error: 'invalid_request',
    ...
    const credentialConfigurationId = parRequest.authorization_details?.[0].credential_configuration_id;
    ...
    return await handleEAAAuthorizationResponse(context, parRequest, request_uri);
  } catch (err) {
