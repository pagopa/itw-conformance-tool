...
describe('GetAuthorizationCodeJwtHandler', () => {
  ...
  it('should update the PAR with code timestamps for EAA', async () => {
    vi.mocked(randomUUID).mockReturnValue('22222222-2222-2222-2222-222222222222');
    ...
    const mockRequest = {
      query: new Map([['request_uri', 'urn:ietf:params:oauth:request_uri:test']])
    } as unknown as HttpRequest;

    const response: HttpResponseInit = await GetAuthorizationCodeJwtHandler(mockRequest, mockContext);
