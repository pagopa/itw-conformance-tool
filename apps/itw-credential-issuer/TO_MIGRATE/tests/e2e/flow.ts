...
import { dpopJwk, dpopJwkPublic, requestProofJwk, requestProofJwkPublic } from './jwk';
...
export class E2EIssuingTestFlow {
  ...
  async fetchFederationMetadata() {
    ...
    const { metadata } = decodeJwt<ItWalletEntityConfigurationClaims>(responseText);
    expect(metadata).not.toBeUndefined();

    const isSupported = isClientAttestationSupported({
      authorizationServerMetadata: metadata.oauth_authorization_server as ItWalletAuthorizationServerMetadata
    });

    expect(isSupported).toEqual({ supported: true });

    this.authorizationServerMetadata = metadata.oauth_authorization_server as ItWalletAuthorizationServerMetadata;
    this.issuerMetadata = metadata.openid_credential_issuer as ItWalletCredentialIssuerMetadata;
  }
  ...
  async openid4vpAuthzRequest() {
    ...
    this.requestObject = decodeJwt<Openid4vpAuthorizationRequestPayload>(authorizationRequestJwt);
  ...
  async presentationResponse() {
    ...
    const pidVpToken = await createMockedVpToken(pidSdJwt.payload, pidSdJwt.header, nonce, client_id);
    ...
    const response = await fetch(`${BASE_URL}/presentation-response?request_uri=${this.requestUri}`, {
      body: formData,
      method: 'POST'
    });
  ...
  async pushedAuthorizationRequest(credentialConfigurationId: string) {
    ...
    const code_challenge = crypto.createHash('sha256').update(this.pkceCode).digest('base64url');
    ...
    const response = await fetch(this.authorizationServerMetadata.pushed_authorization_request_endpoint, {
  ...
  async tokenRequest() {
    ...
    const response = await fetch(this.authorizationServerMetadata.token_endpoint, {
