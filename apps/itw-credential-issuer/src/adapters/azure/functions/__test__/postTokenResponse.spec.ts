...
describe("PostTokenResponseHandler", () => {
  ...
  const mockRequest = {
    ...
    text: vi
      .fn()
      .mockResolvedValue(
        "code=abc&code_verifier=verifier&grant_type=authorization_code",
      ),
  ...
  it("should return 400 invalid_grant when the code is invalid", async () => {
    ...
    const response: HttpResponseInit = await PostTokenResponseHandler(
      mockRequest,
      mockContext,
    );
  ...
  it("should return 500 for unexpected domain errors", async () => {
    vi.mocked(createAccessToken).mockRejectedValueOnce(
      new CreateAccessTokenError("unexpected"),
    );
