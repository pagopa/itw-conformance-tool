      ...
      credential_sets: z
        .array(
          z.object({
            ...
            purpose: z
              .union([z.string(), z.number(), z.record(z.any())])
              .optional(),
...
export type ClaimsObject = Record<
  string,
...
export const responseBody = z
  ...
  .refine(
    (data) =>
      data.response || (data.error && data.error_description && data.state),
    {
      message:
        "Either 'response' or 'error, error_description, state' must be present",
