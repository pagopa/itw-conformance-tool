...
describe('RP local - Erasure request delivery', () => {
  ...
  it('accepts a valid GET erasure request and returns 204 No Content', async () => {
    ...
    await ctx.sessionService.update(state, 'verified', {
      redirectUri: 'https://wallet.example.org/previous-callback',
      values: [{ family_name: 'Rossi', given_name: 'Mario' }]
    });
  ...
  it('rejects unknown query parameters', async () => {
    ...
    const res = await ctx.app.inject({
      method: 'GET',
      url: `/auth/erasure?state=${state}&callback_url=${encodeURIComponent('https://wallet.example.org/erasure-callback')}&unexpected=true`
    });
