...
describe('VP flow — complete issuer ↔ relying party presentation', () => {
  ...
  it('completes the full VP flow end-to-end', async () => {
    ...
    const state = parsedWalletUrl.searchParams.get('state')!;
    const requestUri = parsedWalletUrl.searchParams.get('request_uri')!;
  ...
  it('reports error when nonce is replayed', async () => {
    ...
    const state = new URL(firstReq.json<{ url: string }>().url).searchParams.get('state')!;
    ...
    const state2 = new URL(secondReq.json<{ url: string }>().url).searchParams.get('state')!;
