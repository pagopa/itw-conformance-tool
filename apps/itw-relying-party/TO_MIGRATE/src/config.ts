...
const readFromEnvironment: (variableName: string, env: NodeJS.ProcessEnv) => string = (variableName, env) => {
...
const getConfigFromEnvironment = (env: NodeJS.ProcessEnv) => ({
  ...
  storageAccountConnectionString: readFromEnvironment('storageAccountConnectionString', env)
});
...
export const getConfig: (env: NodeJS.ProcessEnv) => Config = (env) => {
  ...
  const basePath = hostname.startsWith('http://') || hostname.startsWith('https://') ? hostname : `https://${hostname}`;
  return {
    ...config,
    basePath
  };
