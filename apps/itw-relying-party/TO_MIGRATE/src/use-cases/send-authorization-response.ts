...
): (privateKeyBase64: string) => Promise<{ state: string; vpToken: Record<string, string> }> {
  return async function (privateKeyBase64) {
    try {
      const decryptedAuthorizationToken = await decryptAndVerifyJWE(authorizationResponse, privateKeyBase64);
      ...
      throw new BadRequestError('The response cannot be processed because cannot be decrypted or is malformed');
    }
  };
}
...
function verifyWalletAttestation({
  ...
  return async function (clientId) {
    try {
      ...
      const { header: headerWalletAttestation, payload: payloadWalletAttestation } = decodedWalletAttestation;
      ...
      const parsedWalletProviderEC = WalletProviderEC.safeParse(decodedWalletProviderEC);
      if (parsedWalletProviderEC.error) {
        throw new Error();
      }
      const walletProviderKeys = parsedWalletProviderEC.data.metadata.wallet_provider.jwks.keys;
      const walletProviderKey = walletProviderKeys.find((key) => key.kid === kid);
      ...
      try {
        ...
        throw new ForbiddenError('The public key in the issuer signed JWT is malformed');
      }
      ...
      throw new ForbiddenError('Trust could not be established with the Wallet Provider');
    }
  };
}

// 3)
async function getIssuerECFromFetch(issuer: string): Promise<null | z.infer<typeof IssuerEC>> {
  try {
...
function getIssuerECFromTrustChain(issuerSignedJwt: string, issuer: string): null | z.infer<typeof IssuerEC> {
  try {
...
const verifyIssuerSignedJwt: (issuerSignedJwt: string) => Promise<IssuerSignedJwtPayload> = async (issuerSignedJwt) => {
  ...
  try {
    ...
    if (!issuerEC) {
      ...
      console.log(`Fetch from ${issuer}/.well-known/openid-federation failed, falling back to trust_chain`);
    ...
    throw new ForbiddenError('Trust could not be established with the Credential Issuer');
  }
};

// 4) also validations on the token itself not just the signature
function verifyKbJwt(kbJwt: string, holderKey: string): (clientId: string) => Promise<KbJwtPayload> {
  return async function (clientId) {
    try {
      ...
      if (header.typ !== 'kb+jwt') {
        throw new BadRequestError('`typ` value in KB-JWT is not correct. It must be `kb+jwt`');
      }

      const parsedAlg = CryptographicAlgorithms.safeParse(header.alg);
      if (parsedAlg.error) {
        throw new BadRequestError('`alg` value in KB-JWT is not correct. It must be one of the supported algorithms');
      }
      ...
      if (parsedPayload.error) {
        throw new BadRequestError('Invalid KB-JWT: missing required claims in the payload');
      }

      const kbJwtPayload = parsedPayload.data;

      if (kbJwtPayload.aud !== clientId) {
        throw new BadRequestError('Invalid KB-JWT: `aud` does not match RP entity identifier');
      }

      return kbJwtPayload;
    } catch (error) {
      throw error instanceof BadRequestError ? error : new ForbiddenError('The signature of KB-JWT is invalid');
    }
  };
}
...
// 5)
const verifyDisclosuresSdMatch: (disclosures: string[], issuerJwtSd: string[]) => void = (disclosures, issuerJwtSd) => {
  try {
    const areValidDisclosures = disclosures.every((disclosure) => {
      const base64urlDisclosureHash = crypto.createHash('sha256').update(disclosure).digest('base64url');

      return issuerJwtSd.includes(base64urlDisclosureHash);
    });
    if (!areValidDisclosures) {
      throw new BadRequestError('The sd-jwt returned is malformed: disclosures are wrong');
    }
  } catch (error) {
    throw error instanceof BadRequestError
      ? error
      : new BadRequestError('The sd-jwt returned is malformed: disclosures can not be verified');
  }
};

// 6)
const verifySdHash: (params: { disclosures: string[]; issuerJwt: string; sdHash: string }) => void = ({
  disclosures,
  issuerJwt,
  sdHash
}) => {
  try {
    ...
    const isValidSdHash = crypto.createHash('sha256').update(sdString).digest('base64url') === sdHash;
    if (!isValidSdHash) {
      throw new BadRequestError('The sd-jwt returned is malformed: sd hash in KB-JWT is not correct');
    }
  } catch (error) {
    throw error instanceof BadRequestError
      ? error
      : new BadRequestError('The sd-jwt returned is malformed: sd hash can not be verified');
  }
};
...
function verifySdJwt(
  ...
  return async function (clientId) {
    ...
    if (decodedJwt.vct === 'https://pre.ta.wallet.ipzs.it/vct/v1.0.0/WalletAttestation') {
      await verifyWalletAttestation({ disclosures, issuerJwt, kbJwt })(clientId);
    } else {
    ...
    return {
      dcqlPresentation: {
        [sdJwtId]: fromVpTokenToLibraryObjects(decodedDisclosures, decodedJwt.vct, format)
      },
      nonce
    };
  };
...
// 7)
function convertDisclosuresToClaimsObject(disclosures: Disclosure[]): ClaimsObject {
...
const checkPresentation: ({
  ...
  try {
    ...
    throw new BadRequestError('The credential presentation does not have required parameters');
  }
};
...
function checkNonce(localNonces: string[]): (nonceRepository: NonceRepository) => Promise<void> {
    ...
    if (allEqual(localNonces)) {
      try {
        ...
        throw error instanceof NotFoundError
          ? new ForbiddenError('The nonce does not match with the one provided in the request object')
          : error;
      }
    }
  };
...
export function baz1(
  ...
  return async function ({ config, nonceRepository, requestObjectRepository }) {
    const { state, vpToken } = await verifyAuthorizationResponse(bodyResponse)(config.authResponsePrivateKey);

    // 3) / 4) / 5) / 6)
    try {
      const requestObject = await getRequestObject(state)(requestObjectRepository);
      ...
      const resultPromises = Object.entries(vpToken).map(async ([sdJwtId, sdJwt]) => {
        // Try to get format by ID first, then fall back to index-based lookup
        const format = formatById[sdJwtId] ?? formatByIndex[sdJwtId];
        if (!format) {
          throw new BadRequestError(`Unknown credential ID "${sdJwtId}" in VP Token`);
        }
      ...
      const localNonces = result.map(({ nonce }) => nonce).filter((n): n is string => typeof n === 'string');
      ...
      await markRequestObjectAsVerified(state, fullRedirectUri, dcqlPresentation)(requestObjectRepository);
      return {
