import { RequestObjectRepository, deleteRequestObject, getRequestObject } from '@/request-object';

export const getStatus: {
  handler: (state: string) => (requestObjectRepository: RequestObjectRepository) => Promise<{
    redirect_uri: string;
  ...
  handler: (state) => async (requestObjectRepository) => {
    const { redirectUri, status, values } = await getRequestObject(state)(requestObjectRepository);
