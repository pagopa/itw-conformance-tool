...
The paths can be overridden via the `[rp]` section in `config.ini` (`x5c_cert_path`) or through the environment variable `ITW_CT_RP_X5C_CERT_PATH`. The Trust Anchor URL (`trust_anchor_url` / `ITW_CT_RP_TRUST_ANCHOR_URL`) must also be set before starting the RP service.
